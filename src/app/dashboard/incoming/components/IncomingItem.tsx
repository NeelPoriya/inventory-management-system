...
import {
  Data,
  OutgoingItem as IncomingItemType,
} from "@/types/FormattedIncoming";
...
function DeleteItem({ item }: { item: IncomingItemType }) {
  const { toggle: refresh } = useContext(RefreshContext);

  const deleteItem = async () => {
    try {
      const response = await fetch(`/api/info/incoming/${item._id}`, {
        method: "DELETE",
        headers: {
...
function ListItem({ item }: { item: IncomingItemType }) {
  return (
    <Card>
      <CardHeader>
        <CardTitle>
